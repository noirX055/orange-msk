...
export function CatalogView({
  ...
  // Игнорируем выбранные серии, которых больше нет в актуальном списке
  const activeSeries = useMemo(
    () => selectedSeries.filter((series) => seriesList.includes(series)),
  ...
  const visible = useMemo(() => {
    ...
    const filtered = products.filter((product) => {
      ...
      if (activeSeries.length > 0 && !(product.series && activeSeries.includes(product.series)))
        return false
  ...
  }, [products, category, selectedBrands, activeSeries, maxPrice, inStockOnly, saleOnly, sort, query])
          ...
          <div className="flex flex-wrap gap-2">
            {seriesList.map((series) => (
              <button
                ...
                aria-pressed={activeSeries.includes(series)}
