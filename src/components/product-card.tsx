...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-card border border-border bg-card transition-colors hover:border-primary/60">
      <FavoriteButton
        slug={product.slug}
      ...
      <Link href={`/product/${product.slug}`} className="relative block">
        ...
        {!product.inStock && (
          <span className="absolute bottom-3 left-3 rounded-full bg-background/90 px-2.5 py-1 text-xs font-medium text-muted-foreground">
            Нет в наличии
          </span>
        )}
      </Link>
